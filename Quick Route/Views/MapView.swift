...
struct MapView: View {
    ...
    var body: some View {
        Group {
            if let allRoutes = routeViewModel.routes, !allRoutes.isEmpty, let addressPairs = routeViewModel.makeLegAddressPairs() {
                ScrollView {
                    ForEach(Array(allRoutes.enumerated()), id: \.element.polyline) { idx, route in
                        MapCard(route: route, index: idx, addressPair: addressPairs[idx])
                            // Optional: Add padding around each card if desired
                            .padding(.horizontal)
                    }
                }
